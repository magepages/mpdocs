...
* In **Module Enable** field: choose “Yes” to turn the module on.
* In **Gift Code Pattern** field: Is the common pattern of generated codes. When creating a Gift Code or a Gift Code Pool, admins can change this template as their desire.
* In **Enable Gift Card Credit** field: Select “Yes” so the customer can use their Gift Card. Then **Gift Card Can Be Redeem** section will be displayed.
* In **Gift Card Can Be Redeem** field: 
...
* In **Gift Card Lifetime** field: It’s the place to set the expiration date of a gift card purchased by a customer. 
* In **Enable Delivery Time** field: 
...
* In **Customer Can Select Timezone** field:
...
* In **Gift Card Fields Show On Item** field:
...
* In **Create Gift Code(s) When** field
...
* In **Order Completed** line: The gift card of customer will be created as soon as the **Order Complete** is accomplished. The gift code will be stored in **Manage Gift Codes**
...
* In **Use Coupon Box To Apply Gift Card** field:
...
* In **Can Use Multiple Gift Cards** field:
...
* In **Show Gift Card Summary On Total Block** field:
...
* In **Can Use For Shipping Amount**: Select “Yes” to apply a gift Card and gift credit for the shipping fee. If both gift card and gift credit are having sufficient value and the customer doesn’t purchase Gift Card Product, the order can be considered as a free one.
* In **Allow Refund Gift Card**: Select “Yes” to be able to refund amount of a gift card or gift credit on its balance.
...
* In **Logo** field: choose your own store’s logo. If a logo is already included in the Template, then it will be applied on customer’s gift card.
* In **Maximum message character** field: 
...
* In **Enable Email Notification** section: choose “Yes” to enable the function and “No” for vice versa result. When a gift card arrived to recipients perfectly, an email containing the gift card will be informed back to the sender.
* In **Email sender** field: Available 5 Magento’s default email senders for your selection: General Contact, Sales Representative, Customer Support, Custom Email 1, Custom Email 2
* In **Gift Card Email Template** field:
...
* In **Enable** field: choose “Yes” to turn the function on.
* In **Gift Card Update Email Template** function: similar to **Gift Card Email Template**
...
* In **Before Update Email Template** function: similar to **Gift Card Email Template**
* In **Before Expired Day(s)** field:
...
* In **Enable** field: choose “Yes” to turn the function on.
* In **Sender Confirmation Email Template** function: similar to **Gift Card Email Template**
...
* In **Unused Notification Email Template** function: similar to **Gift Card Email Template**
* In **After Unused Day(s)**
...
* In **Enable** field: Select “Yes” so that customers can receive emails every time the balance of a gift credit is updated
* In **Update Balance Email Template** field: similar to **Gift Card Email Template**
...
* In **Enable SMS Delivery** field: Select “Yes” to allow the system to send SMS about the gift card message to customers.
* In **Twilio Account SID**: Admin needs to register a Twilio Account, use the SMS sending method to enter the correct SID of his Twilio Account
* In **Twilio Account Token** field: Along with its SID, admin also needs to fill out the correspondent Token Twilio Account
* In **Sender Phone Number**: The field to enter a phone number to send. The phone number should be the one purchased from Twilio
* In **SMS Message** field: This is where the admin modifies the content of the SMS before sending it to customers. Admin can use premade variables to add a SMS text.
...
In **Gift Card Update Email Template** field: Admin can use those one of available email templates
...
* In **Message** field: Admin can use pre-made variables in the above box to change the SMS content.
...
* In **Status** field: Select “Active” to display Template in the Gift Card Product Page frontend, in case Gift Card Product selects this template
* In **Allow Upload Image**, if the Template is displayed in the Gift Card Product frontend
...
* In **Gift Card Title** field:
  ...
  * If you leave it blank, there will be an error message
* In **Font Family** field: Admin can change fonts with some pre-installed fonts in this section, including Times-Roman, Helvetica and Courier.
* In **Background Image**: Admin change the background image in this section. If it’s blank, gift card will have a white background
* In **Gift Card Note** field:
...
* In **Store** field: the specific store view which a gift code is used. A website can have multiple store views. The gift code can only be used in the Store View which admins choose.
* In **Expires At** field: admins can set the expiration date for a gift code at here. Gift code can not be used if it’s overdue.
* In **Created At** section: only visible when gift code was created. It includes date and time creating.
* In **Created From** field: This entry is only visible to gift codes created from the Order that contains the Gift Card Product, including the Order ID and the name of the Gift Card Product.
...
* In **Template** field: The generated templates will be displayed here. When you select the desired template, its image will be displayed (if the Template has image)
* In **Image** field: This part is visible when the above Template is selected. Admins can change images to print on the gift card. The image that customers upload when buying Gift Card Product is also shown in this section.
* In **Sender Name** field:
...
* In **Name** field: A friendly note that you need to name the gift code pool to distinguish easily from other pools.
* In **Status** field: Choose “Active” to enable the pool .
* In **Can Inherit** field: Select “Yes” so that created gift codes will also be changed according to the new alternative of the Pool (except for the Status Attribute)
...
* In **Code Pattern**: Admins can change the code pattern of the pool. The created gift card will have the same gift code form.
* In **Gift Card Qty**: Admins simply enter the number of gift card you want to create and click ``Generate`` to get code quickly
